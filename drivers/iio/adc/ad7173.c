...
#define AD7173_GPIO_GP_DATA1		BIT(7)
#define AD7173_GPIO_GP_DATA0		BIT(6)

#define AD7173_SETUP_BIPOLAR		BIT(12)
#define AD7173_SETUP_AREF_BUF		(0x3 << 10)//bit11 REFBUF0+ : 1 bit10 REFBUF0− : 1
#define AD7173_SETUP_AIN_BUF		(0x3 << 8)//bit[9:8] INBUF0 : 11
				  ...
				  unsigned int channel)
{
	unsigned int config;
	int ret= 0;
	unsigned int setupBef;
	config = AD7173_SETUP_REF_SEL_INT_REF;
    // config = AD7173_SETUP_REF_SEL_EXT_REF2;

	if (ch->differential)
		config |= AD7173_SETUP_BIPOLAR;
    // printk("aodg-config1 : 0x%x\n",config);
    // printk("aodg-prepare channel :%d!\n",channel);
	// if(channel==0)
	// {
	// 	config |= AD7173_SETUP_AIN_BUF;//Enable the input buffers in the setup register for voltage input channels.
	// }
    // config =  0x0020;
	config |= AD7173_SETUP_AREF_BUF | AD7173_SETUP_AIN_BUF;
    // printk("aodg-config2 : 0x%x\n",config);
	ret = ad_sd_read_reg(sd, AD7173_REG_SETUP(channel), 2, &setupBef);
	if (ret)
		return ret;
    // printk("aodg-setupBef: 0x%x\n", setupBef);

	return ad_sd_write_reg(sd, AD7173_REG_SETUP(channel), 2, config);
}
...
static int ad7173_set_channel(struct ad_sigma_delta *sd, unsigned int channel)
{
	...
	*/
	...
	// val = AD7173_CH_ENABLE | ch->ain;
	val = AD7173_CH_ENABLE | ch->ain | AD7173_CH_SETUP_SEL(channel);//aoao
    // printk("aodg-ch val:0x%x , ch->ain:0x%x ",val, ch->ain);
    // printk("aodg-set channel:%d !\n", channel);
...
{
	struct ad7173_state *st = ad_sigma_delta_to_ad7173(sd);
	// printk("aodg-enum adc mode =%d\n",mode);
	// mode = AD_SD_MODE_SINGLE;
	st->adc_mode &= ~AD7173_ADC_MODE_MODE_MASK;
	st->adc_mode |= AD7173_ADC_MODE_MODE(mode);
    // printk("aodg-adc Mode :0x%x  set mode !\n",st->adc_mode);
...
static int ad7173_setup(struct iio_dev *indio_dev)
{
	...
	unsigned int id;
	unsigned int ifMode;//interface mode
	unsigned int ifModeConfig = 0;//interface mode
	uint8_t *buf;
	int ret;
	...
	if (ret)
		return ret;
    printk("aodg-id1: 0x%x\n", id);
	id &= AD7173_ID_MASK;
    printk("aodg-id2: 0x%x\n", id);
	...
	ret = ad_sd_read_reg(&st->sd, AD7173_REG_INTERFACE_MODE, 2, &ifMode);
	if (ret)
		return ret;
    printk("aodg-ifMode1: 0x%x\n", ifMode);
	
	ifModeConfig |= BIT(6) | BIT(0);//bit6 DATA_STAT 1 ; bit0 WL16 1
	ret = ad_sd_write_reg(&st->sd, AD7173_REG_INTERFACE_MODE, 2, ifModeConfig);
	if (ret)
		return ret;
    printk("aodg-ifModeAft: 0x%x\n", ifModeConfig);

	ret = ad_sd_read_reg(&st->sd, AD7173_REG_INTERFACE_MODE, 2, &ifMode);
	if (ret)
		return ret;
    printk("aodg-ifMode2: 0x%x\n", ifMode);

	printk("aodg-adc_mode before: 0x%x\n", st->adc_mode);
	// st->adc_mode |= AD7173_ADC_MODE_REF_EN  | AD7173_ADC_MODE_CLOCKSEL(0x3);//go 0x800c
	st->adc_mode |= AD7173_ADC_MODE_REF_EN  | AD7173_ADC_MODE_CLOCKSEL(0) | AD7173_ADC_MODE_SING_CYC;//ao 0x8300
    printk("aodg-adc_mode: 0x%x\n", st->adc_mode);
	return 0;
}
	...
	struct device_node *np)
{
	...
	if (!st->channels)
    {
        printk("aodg-st->channels failed!\n");
		return -ENOMEM;
	...
	if (st->info->has_temp &&skip_temp_channel_processing ==false) {
	...
	for_each_available_child_of_node(chan_node, child) {
        ...
        printk("aodg-reg: %u %u\n", ain[0], ain[1]);
		// ret = of_property_read_u32_array(child, "reg", vin, 2);
		// if (ret) {
		// 	of_node_put(chan_node);
		// 	of_node_put(child);
		// 	return ret;
		// }
        // printk("aodg-reg2: %u %u\n", vin[0], vin[1]);
        ...
        // if (vin[0] >= st->info->num_inputs ||
		//     vin[1] >= st->info->num_inputs) {
		// 	dev_err(indio_dev->dev.parent,
		// 		"Input pin number out of range.\n");
		// 	of_node_put(chan_node);
		// 	of_node_put(child);
		...
		*chan = ad7173_channel_template;
        ...
        printk("aodg-st->channels[%u].ain: %u\r\n", chan_idx, st->channels[chan_idx].ain);
        printk("aodg-st->channels[%u].differential: %d\r\n", chan_idx, st->channels[chan_idx].differential);
...
static int ad7173_probe(struct spi_device *spi)
{
	...
	printk("aodg-spi mode: %d\n", spi->mode);
    spi->mode = SPI_MODE_3;
    // printk("aodg-cs0 gpio: %p\n", spi->cs_gpio);
    // spi->cs_gpio = 5;
    // printk("aodg-cs1 gpio : %d\n",spi->cs_gpio);
    ...
    // printk("aodg-indio_dev->info->data_reg:%u\n",indio_dev->info.)
	ret = ad7173_of_parse_channel_config(indio_dev, spi->dev.of_node);
	if (ret)
		return ret;
	printk("aodg-devm_ad_sd_setup_buffer_and_trigger");
